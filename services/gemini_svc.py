...
async def process_expense_text(input_text: str, preferred_currency: str = "GBP"):
    ...
    CURRENCY (if unspecified, default is {preferred_currency}. Assume that $ is SGD, not USD. Make sure to return only the 3-letter symbol (example: GBP, SGD, EUR, JPY, MYR, RMB));
...
# implement exponential backoff for load handling
@retry(wait=wait_random_exponential(multiplier=1, max=60))
async def process_expense_image(image_path: str, caption: str="", preferred_currency: str = "GBP"):
    ...
    - For currency, default to {preferred_currency} if unspecified. Assume $ means SGD unless context suggests otherwise.
